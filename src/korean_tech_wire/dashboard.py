...
def render_overview(database: Database, sources: dict[str, object], query: dict[str, list[str]], collection: dict[str, object] | None = None, can_collect: bool = False, archive: QCArchive | None = None, show_experimental: bool = False) -> str:
    ...
    # STD-UI-COM-009: the backend keeps per-run, phase-attributable outcomes
    # (run_errors.error_type per failing source, plus the extraction/rejection
    # counters in summary_json). A bare "partial_failure" badge erases which
    # phase actually broke, so the two columns below carry that attribution
    # inline -- discovery/collector errors keyed by type and source, and the
    # post-discovery phase counters, which are a different remediation.
    with database.connect() as con:
        runs = con.execute("SELECT id,source_id,started_at,status,summary_json FROM runs ORDER BY id DESC LIMIT 20").fetchall()
        error_rows = con.execute("SELECT run_id,source_id,error_type FROM run_errors ORDER BY id").fetchall()
    errors_by_run: dict[int, list[str]] = {}
    for row in error_rows:
        errors_by_run.setdefault(row["run_id"], []).append(f'{row["source_id"]}: {row["error_type"]}')
    run_rows = []
    for r in runs:
        summary = json.loads(r["summary_json"] or "{}")
        phases = []
        if summary.get("extraction_failed"): phases.append(f'{summary["extraction_failed"]} extraction')
        if summary.get("rejected"): phases.append(f'{summary["rejected"]} rejected')
        if summary.get("timestamped") is not None and summary.get("accepted"):
            phases.append(f'{summary["timestamped"]}/{summary["accepted"]} timestamped')
        run_rows.append((
            e(sources.get(r["source_id"]).name if r["source_id"] in sources else r["source_id"] or "all"),
            e(r["started_at"]), badge((r["status"] or "UNKNOWN").upper()),
            e(summary.get("discovered", "—")), e(summary.get("new", "—")),
            e("; ".join(errors_by_run.get(r["id"], [])) or "—"),
            e(" · ".join(phases) or "—"),
        ))
    qc_rows = []
