...
1. **SK hynix Korea `403 Forbidden` from the Hetzner egress IP** — open production incident, source of the entire soak's zero-yield for the sole original production source. Needs investigation independent of Stage 4 (contact SK hynix, try a different egress path, or reassess whether this IP range is broadly blocklisted).
2. **`soak --if-due` due-gating defect** — a single persistently-failing source prevents the gate from ever reporting "not due," so the whole fleet has been polled every 30 minutes instead of the documented 2 hours since 2026-08-10. Recommend evaluating due-ness per source rather than requiring every selected source to have a fresh success.
3. **Samsung Newsroom Korea has no editorial filter** — confirmed code gap (see REWORK decision above); needs a dedicated low-value-term filter before re-evaluation.
4. **LG Display's mid-soak connection-reset pattern** — self-resolved (47 consecutive successes at closeout) but worth a few more cycles of confirmation before treating access as fully settled.
